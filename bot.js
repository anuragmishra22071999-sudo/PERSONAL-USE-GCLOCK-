...
if(!ADMIN_ARG){ console.error("❌ Missing admin UID"); process.exit(1); }
...
if(!fs.existsSync(USER_DIR)){ console.error("❌ User folder not found"); process.exit(1); }

let appState;
try{ appState = JSON.parse(fs.readFileSync(APPSTATE_PATH,"utf8")); }catch(e){ console.error("❌ Failed reading appstate.json",e.message); process.exit(1); }
...
try{ if(fs.existsSync(LOCKS_PATH)) locks=JSON.parse(fs.readFileSync(LOCKS_PATH,"utf8")); }catch(e){ console.warn("⚠️ Could not load locks.json"); }

function saveLocks(){ try{ fs.writeFileSync(LOCKS_PATH,JSON.stringify(locks,null,2)); }catch(e){ console.error(e.message); } }
...
async function enforceNickLock(api, threadID, nick){
  try{
    ...
    try{
      ...
      if(event.type==="event"){
        ...
        if(logType==="log:thread-name"){
          ...
          if(lockedName && newName!==lockedName){ try{ await api.setTitle(lockedName,threadID); log(`🔒 Reverted thread name`);}catch{} }
          return;
        }
        // nickname revert
        ...
        // anti-out
        if(["log:unsubscribe","log:remove","log:user-left","log:remove-participant"].includes(logType)){
          ...
          if(leftUID && locks.antiOut?.[threadID] && leftUID!==api.getCurrentUserID()){
            try{ await api.addUserToGroup(leftUID, threadID); await api.sendMessage(`🚨 Anti-Out: Added back ${leftUID}`,threadID); log(`🚨 Added back ${leftUID}`);}catch(e){log(e.message);}
          }
          return;
        }
      }
      ...
      if(rawCmd==="nicklock"){
        ...
        if(sub==="on"){
          ...
          if(!nick) return api.sendMessage("⚠️ Usage: /nicklock on <Nick>",threadID);
          await enforceNickLock(api, threadID, nick);
